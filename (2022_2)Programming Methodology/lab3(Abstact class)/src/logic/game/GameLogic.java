...
public class GameLogic {
	...
	public ArrayList<BaseCard> draw(int amount) {
		// TODO Implement here
		ArrayList<BaseCard> allcardtoadd = new ArrayList<BaseCard>();
		if (amount > this.getDeck().size()) {
			allcardtoadd.addAll(this.getDeck());
			this.getCurrentPlayerHand().addAll(allcardtoadd);
			this.getDeck().clear();
		} else {
			while (allcardtoadd.size() < amount) {
				allcardtoadd.add(this.getDeck().get(0));
